...
class TestF1Score(object):
    ...
    def test_config(self):
        r_obj = metrics.F1Score(
            name='my_f1', thresholds=[0.4, 0.9], top_k=15, class_id=12)
        assert r_obj.name == 'my_f1'
        assert len(r_obj.weights) == 3
        assert ([v.name for v in r_obj.weights] ==
        ...
        # Check save and restore config
        r_obj2 = metrics.F1Score.from_config(r_obj.get_config())
        assert r_obj2.name == 'my_f1'
        assert len(r_obj2.weights) == 3
        assert r_obj2.thresholds == [0.4, 0.9]
        assert r_obj2.top_k == 15
        assert r_obj2.class_id == 12
    ...
    def test_unweighted_all_incorrect(self):
        r_obj = metrics.F1Score(thresholds=[0.5])
        inputs = np.random.randint(0, 2, size=(100, 1))
        y_pred = K.constant(inputs)
        y_true = K.constant(1 - inputs)
        result = r_obj(y_true, y_pred)
    ...
    def test_unweighted_with_threshold(self):
        r_obj = metrics.F1Score(thresholds=[0.5, 0.7])
        y_pred = K.constant([1, 0, 0.6, 0], shape=(1, 4))
        y_true = K.constant([0, 1, 1, 0], shape=(1, 4))
        result = r_obj(y_true, y_pred)
        assert np.allclose([0.5, 0.], K.eval(result), 0)
    ...
    def test_weighted_with_threshold(self):
        r_obj = metrics.F1Score(thresholds=[0.5, 1.])
        y_true = K.constant([[0, 1], [1, 0]], shape=(2, 2))
        y_pred = K.constant([[1, 0], [0.6, 0]],
                            shape=(2, 2),
                            dtype='float32')
    ...
    def test_unweighted_class_id(self):
        r_obj = metrics.F1Score(class_id=2)
        ...
        assert np.isclose(1, K.eval(r_obj.true_positives))
        assert np.isclose(0, K.eval(r_obj.false_negatives))
        assert np.isclose(0, K.eval(r_obj.false_positives))
        ...
        assert np.isclose(1, K.eval(r_obj.true_positives))
        assert np.isclose(1, K.eval(r_obj.false_negatives))
        assert np.isclose(1, K.eval(r_obj.false_positives))

    def test_unweighted_top_k_and_class_id(self):
        r_obj = metrics.F1Score(class_id=2, top_k=2)
    ...
    def test_unweighted_top_k_and_threshold(self):
        r_obj = metrics.F1Score(thresholds=.7, top_k=2)
        ...
        K.eval(result)
        assert np.isclose(f1, K.eval(result))
        assert np.isclose(2, K.eval(r_obj.true_positives))
        assert np.isclose(6, K.eval(r_obj.false_negatives))
        assert np.isclose(2, K.eval(r_obj.false_positives))
